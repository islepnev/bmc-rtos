    ...
    if (d && d->priv) {
        ...
        for (uint32_t i=0; i<VXSIIC_SLOTS; i++) {
            ...
            if (0 == status->present)
                ...
                printf("%2s %s%s%s %4lu %4lu %8s  %02X  %04X-%04X %8s %7lu  %s  %s  %s  %s  %s  %s\n",
                       label,
                       (status->ioexp & VXSIIC_PP_IOEXP_BIT_INITB) ? "." : "I",
                       (status->ioexp & VXSIIC_PP_IOEXP_BIT_PGOOD) ? "P" : ".",
                       (status->ioexp & VXSIIC_PP_IOEXP_BIT_DONE) ? "D" : ".",
                       (unsigned long)status->iic_master_stats.errors,
                       (unsigned long)status->mcu_info.iic_stats.errors,
                       bmc_ver_buf,
                       (uint8_t)(status->mcu_info.module_id & 0xFF),
                       (uint16_t)(status->mcu_info.module_serial >> 16),
                       (uint16_t)(status->mcu_info.module_serial & 0xFFFF),
                       fw_ver_buf,
                       (unsigned long)status->mcu_info.uptime,
