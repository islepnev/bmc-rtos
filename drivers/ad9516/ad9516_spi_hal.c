...
static bool ad9516_read1_duplex(uint16_t reg, uint8_t *data)
{
    enum {Size = 3};
    uint8_t tx[Size];
    uint8_t rx[Size];
...
void ad9516_test_loop(void)
{
    ...
    while (1) {
        //sdata = ~data;
        ad9516_write1_internal(reg, data);
        uint8_t read = 0;
...
bool ad9516_write1(uint16_t reg, uint8_t data)
{
    if (! ad9516_write1_internal(reg, data))
        return false;
    if (! ad9516_write1_internal(0x232, 1))
...
bool ad9516_write_config(uint8_t data)
{
