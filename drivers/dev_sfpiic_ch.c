...
{
    sfpiic_ch_status_t *status = &d->status.ch[pp];
    if (HAL_OK == ret) {
...
static HAL_StatusTypeDef dev_sfpiic_ch_enable_tx(Dev_sfpiic *d, int ch)
{
    ...
    uint16_t reg = 86;
    uint8_t val = 1;
    ret = sfpiic_mem_read(reg, &val, 1);
    if (HAL_OK != ret)
        return ret;
    if((val&0xF)==0)
        return ret;

    ++d->status.ch[ch].tx_en_cnt;
    val &= ~0xF;
    ret = sfpiic_mem_write(reg, &val, 1);
    if (HAL_OK != ret)
        return ret;
    val=1;
    ret = sfpiic_mem_read(reg, &val, 1);
...
{
    HAL_StatusTypeDef ret = HAL_OK;
    const uint16_t reg = 22;
    uint8_t data[2] = {0};
    ret = sfpiic_mem_read(reg, data, 2);
    dev_sfpiic_update_ch_state(d, ch, ret);
    if (HAL_OK == ret) {
        int16_t *temp = &d->status.ch[ch].temp;
...
{
    HAL_StatusTypeDef ret = HAL_OK;
    const uint16_t reg = 26;
    uint8_t data[2] = {0};
    ret = sfpiic_mem_read(reg, data, 2);
    dev_sfpiic_update_ch_state(d, ch, ret);
    if (HAL_OK == ret) {
        uint16_t *volt = &d->status.ch[ch].volt;
        *volt = (uint16_t)(data[0]<<8)| data[1];
...
static HAL_StatusTypeDef dev_sfpiic_ch_read_vendor_serial(Dev_sfpiic *d, int ch)
{
    ...
    char buf[16];
    ret = sfpiic_mem_read(reg, (uint8_t *)&buf[0], size);
    dev_sfpiic_update_ch_state(d, ch, ret);
    if (HAL_OK == ret) {
        memcpy(d->status.ch[ch].vendor_serial, buf, sizeof (buf));
...
static HAL_StatusTypeDef dev_sfpiic_ch_read_vendor_name(Dev_sfpiic *d, int ch)
{
    HAL_StatusTypeDef ret = HAL_OK;
    uint16_t reg;
    reg = 148; // Vendor name (16 bytes) QSFP vendor name (ASCII)
    const size_t size = 16;
    char buf[16];
    ret = sfpiic_mem_read(reg, (uint8_t *)&buf[0], size);
    dev_sfpiic_update_ch_state(d, ch, ret);
    if (HAL_OK == ret) {
        memcpy(d->status.ch[ch].vendor_name, buf, sizeof (buf));
