...
bool fpgaWriteSensors(void)
{
    ...
    if (! fpga_spi_hal_write_reg(address++, (int16_t)(sensors->arr[SENSOR_VXS_5V].priv.busVoltage * 1000)))
        return false;
    if (! fpga_spi_hal_write_reg(address++, (int16_t)(sensors->arr[SENSOR_VXS_5V].priv.current * 1000)))
        return false;
