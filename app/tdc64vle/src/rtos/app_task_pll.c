...
#include "log/log.h"
#include "tqdc_clkmux/dev_tqdc_clkmux_fsm.h"
...
static Dev_tqdc_clkmux clkmux = {0};
...
static void run(void)
{
    ...
    dev_tqdc_clkmux_run(&clkmux);
    const bool power_on = enable_power && system_power_present;
    dev_ad9545_run(&pll, power_on);
    main_clock_ready = tqdc_clkmux_running(&clkmux) && ad9545_running(&pll);
}
