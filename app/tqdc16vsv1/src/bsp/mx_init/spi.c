...
{
    // AD9548: CPOL=0, CPHA=0, MSB first, 4-wire
    SPI_HandleTypeDef *hspi = hspi_handle(index);
    hspi->Instance = spi_instance(index);
    hspi->Init.Mode = SPI_MODE_MASTER;
    hspi->Init.Direction = SPI_DIRECTION_2LINES; // no MISO on TDC72VHL-v2
...
void HAL_SPI_MspInit(SPI_HandleTypeDef* spiHandle)
{
    ...
    if (spiHandle->Instance==SPI1)
    {
        ...
        GPIO_InitStruct.Mode = GPIO_MODE_AF_PP; // GPIO_MODE_AF_OD for half-duplex
