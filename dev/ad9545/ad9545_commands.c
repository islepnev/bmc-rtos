...
const int64_t phaser_step_ps = 100;
...
void setup_phaser(Dev_ad9545 *d)
{
    ...
    if (pll_clock_shift_denom != 0 && pll_clock_shift_numer != 0)
        phaser.stop_value += (uint64_t)16000 * pll_clock_shift_numer / pll_clock_shift_denom;

    phaser.req = pll_clock_shift_req;
...
{
    if (phaser_complete()) {
        if (pll_clock_shift_req != pll_clock_shift_ack)
        setup_phaser(d);
        pll_clock_shift_ack = phaser.req;
    }
    if (phaser_complete())
        return true;
